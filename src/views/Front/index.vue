<template>
  <container
    ...
    <template
      ...
      slot="headerMenu">
      <h-menu-template
        v-for="(item, index) of getFrontMenu"
        v-if="(!item.publicOnly || (item.publicOnly && !isUserLoggedIn)) && (!item.privateOnly || (item.privateOnly && isUserLoggedIn))"
        :item="item"
    ...
    <template slot="footerNavigationLeft">
      <template v-for="(block, index) of getFrontFooterNavigationLeft">
        <h3
          :key="'front_footer_navigationl_block_' + index + '_head'"
          class="h5 font-w700"
          v-text="block.name"/>
        <ul
          :key="'front_footer_navigationl_block_' + index + '_list'"
          class="list list-simple-mini font-size-sm">
          <li
            v-for="(child, idx) of block.children"
            :key="'front_footer_navigationl_block_' + index + '_list_item_' + idx">
            <a
              :href="child.link"
    ...
    <template slot="footerNavigationRight">
      <template v-for="(block, index) of getFrontFooterNavigationRight">
        <h3
          :key="'front_footer_navigationr_block_' + index + '_head'"
          class="h5 font-w700"
          v-text="block.name"/>
        <ul
          :key="'front_footer_navigationr_block_' + index + '_list'"
          class="list list-simple-mini font-size-sm">
          <li
            v-for="(child, idx) of block.children"
            :key="'front_footer_navigationr_block_' + index + '_list_item_' + idx">
            <a
              :href="child.link"
              class="link-effect font-w600"
              v-text="child.name"/>
